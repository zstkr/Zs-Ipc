...
> **重要提示 (Important Note)**: 
> 本目录下的 `dkms-install.sh`、`dkms-remove.sh` 以及下方原版说明中的普通 `make` 命令，仅适用于 PC（Ubuntu/Kali）或树莓派本地编译，**不适用于 zs-ipc RV1106 平台**。
> 
> 如果您是要为 `zs-ipc` 项目编译网卡驱动，请严格按照以下步骤操作：

### 步骤 1：使用定制一键编译脚本
1. 在本目录下找到 `build_driver.sh` 脚本。
2. 用编辑器打开它，将 `LUCKFOX_SDK_PATH` 变量修改为您本地的 Luckfox SDK 绝对路径。
3. 在终端中运行该脚本进行交叉编译：
   ```bash
